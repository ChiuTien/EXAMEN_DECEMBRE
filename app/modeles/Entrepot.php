...
class Entrepot extends Model {
    ...
    public function getAll() {
        $DB = parent::db();
        $q = $DB->prepare("SELECT * FROM Entrepot");
        $q->execute();
    ...
    public function getById($id) {
        $DB = parent::db();
        $q = $DB->prepare("SELECT * FROM Entrepot WHERE id = :id");
    ...
    public function save() {
        $DB = parent::db();
        if(!empty($this->id)) {
            $q = $DB->prepare("UPDATE Entrepot SET val = :val WHERE id = :id");
            $q->bindValue(':val',$this->addresse,PDO::PARAM_STR);
            $q->bindValue(':id',$this->id,PDO::PARAM_INT);
            return $q->execute();
        }
        $q = $DB->prepare("INSERT INTO Entrepot (val) VALUES (:val)");
        $q->bindValue(':val',$this->addresse,PDO::PARAM_STR);
        $res = $q->execute();
    ...
    public function delete() {
        ...
        $DB = parent::db();
        $q = $DB->prepare("DELETE FROM Entrepot WHERE id = :id");
        $q->bindValue(':id',$this->id,PDO::PARAM_INT);
        return $q->execute();
    }
}
