...
class Statut extends Model {
    ...
    public function getAll() {
        $DB = parent::db();
        $q = $DB->prepare("SELECT * FROM Statut");
        $q->execute();
    ...
    public function getById($id) {
        $DB = parent::db();
        $q = $DB->prepare("SELECT * FROM Statut WHERE id = :id");
    ...
    public function save() {
        $DB = parent::db();
        if(!empty($this->id)) {
            $q = $DB->prepare("UPDATE Statut SET val = :val WHERE id = :id");
            $q->bindValue(':val',$this->val,PDO::PARAM_STR);
            $q->bindValue(':id',$this->id,PDO::PARAM_INT);
            return $q->execute();
        }
        $q = $DB->prepare("INSERT INTO Statut (val) VALUES (:val)");
        $q->bindValue(':val',$this->val,PDO::PARAM_STR);
        $res = $q->execute();
    ...
    public function delete() {
        ...
        $DB = parent::db();
        $q = $DB->prepare("DELETE FROM Statut WHERE id = :id");
        $q->bindValue(':id',$this->id,PDO::PARAM_INT);
        return $q->execute();
    }
}
